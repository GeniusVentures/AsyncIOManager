...
# ----------------
    ...
    getURLComponents( "https://www.example.com/test.jpg", url_prefix, file_path, extension );
    ...
    auto protocols = libp2p::protocol::factory::ProtocolFactory::createProtocols( libp2phost,
                                                                                  protocol_config,
                                                                                  injector );
    protocols.identify->start();

    // Setup host listening address
    auto bindaddress = ( boost::format( "/ip4/%s/tcp/%d/p2p/%s" ) % "0.0.0.0" % 40000 % libp2phost->getId().toBase58() )
                           .str();
    ...
    auto server_addr = libp2p::multi::Multiaddress::create(
        ...
        if ( getURLComponents( file_name, prefix, base, extension ) && prefix == "ipfs" )
        {
            ...
            if ( parseIPFSUrl( base, cid_str, file_component ) )
            {
                ...
                if ( cid_result.has_value() )
                {
                    ...
                    std::cout << "Added IPFS provider for CID: " << cid_str << " (file: " << file_name << ")"
                              << std::endl;
                }
                else
                {
